...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (file && file.name) {
      ...
      if (company.customer_img === file.name) {
        filename = company.customer_img;
      } else {
        // If there's an existing image, delete it
        if (company.customer_img) {
          const oldImagePath = path.join(process.cwd(), "public/images/", company.customer_img);
      ...
      filename = company.customer_img; // Retain the existing image if no new file is uploaded
    ...
    const updatedCustomer = await Customer.findByIdAndUpdate(
      params.id,
      { $set: { heading, description, customer_img: filename } },
      { new: true }
    );
